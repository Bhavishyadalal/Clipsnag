...
# ---------- Rate Limiting (Critical for longevity) ----------
limiter = Limiter(
    app,
    key_func=get_remote_address,
    default_limits=["5 per minute", "50 per hour"]  # Per IP
)
...
# ---------- Core Fetch Function with Fallback Chain ----------
def fetch_formats_with_fallback(url):
    """
    Tries multiple authentication strategies in order.
    Returns (formats, error_message)
    """
    strategies = []

    # Strategy 1: PO Token + TV Client (Best)
    ...
    # Strategy 3: Android Client (No auth, works but limited to 1080p)
    strategies.append({
...
def download_endpoint():
    ...
    # Build command with the same fallback logic (simplified: try PO Token first, then fallback)
